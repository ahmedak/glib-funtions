...
Path * graph_dijkstra(Graph * g, int src){
	Path * dj =(Path *)malloc(sizeof(struct _path));
	dj->distance =(int *)malloc((g->V)*sizeof(int));
	int v = g->V;
	dj-> v = v;
	int ** graph = g->am;
	int dist[v];
...
Mst * graph_mst_kruskal(Graph * graph){
    	Mst * m =(Mst *) malloc(sizeof(struct _mst));
			int V = graph->V;
	...
	m->e = e;
	m->edge = (Edge *)malloc(e * sizeof(struct _Edge));
	for(i = 0; i < e ;i++)
		m->edge[i] = result[i];
	return m;
}
